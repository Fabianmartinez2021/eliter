//export const apiUrl = 'http://200.6.157.80:4000';
export const apiUrl = 'http://190.9.40.111:4000';

export const passphrase = 'app-development-22052020';
//frase de contraseÃ±a para data de ventas offline
export const passphraseData = 'data-development-sale-14082020';
export const passphraseSale = 'sale-development-sale-14082020';
//frase de contraseÃ±a para data de ventas offline
export const passphraseDataFiscal = 'data-fiscal-development-sale-14082020';
export const passphraseSaleFiscal = 'sale-fiscal-development-sale-14082020';
export const phoneAdmin = '+584121067055';

/** En false: no se muestran menú, atajos ni botones del módulo fiscal/ventas especiales (rutas y servicios siguen en el proyecto). */
export const SHOW_SPECIAL_SALES_MODULE = false;

/** En false: cupones, ofertas/promociones (menú, rutas, UI en ventas y reportes). El código permanece en el proyecto. */
export const SHOW_COUPONS_AND_PROMOTIONS_MODULE = false;
