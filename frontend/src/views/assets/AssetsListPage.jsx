...
function AssetsListPage() {
	...
	useEffect(() => {
		if(assets.deleted){
			dispatch(assetsActions.getList(getUserData(), 1, perPageSelect == 0 ? perPage : perPageSelect, direction, filters));
		}
  	},[assets.deleted]);
	...
	const columns = [
		...
		},
		{
		   name: '',
		   button: true,
		   omit: false, //((user.role !== 1) && (user.role !== 2)),
		   cell: row => {
				return <><Button color="primary btn-round" size="sm" style={{fontSize:12}} disabled={loadingPage || deletingAssets} onClick={e => 
					{
						e.preventDefault(); 
						history.push('/update-assets',{id:row._id})
					}
				}>Editar
				</Button>
		</>
	   	...
	   	},
	   	{
			...
			cell: row => {
				if((user.role === 1) || (user.role === 2) || (user.role === 9)){
					return <><Button color="primary btn-round" size="sm" style={{fontSize:12}} disabled={deletingAssets} onClick={e => 
								{
									setDeleteModalOpen(true);
									setDataToDelete(row);
								}
							}>{deletingAssets && <span className="spinner-border spinner-border-sm mr-1"></span>}<i className="fa fa-trash" aria-hidden="true"></i>
							</Button>
						</>
				}
			},
	...
	const headers = [
		...
		{ label: "Dias en reparación", key: "daysUnderRepair" },
	...
	const getDataTable = (page) => {
		dispatch(assetsActions.getList(getUserData(), page, perPageSelect == 0 ? perPage : perPageSelect, direction, {}));
	}

	//Paginar
	const handlePageChange = async (page) => {
		dispatch(assetsActions.getList(getUserData(), page, perPageSelect == 0 ? perPage : perPageSelect, direction, filters ? filters: {}));
	};
	...
	const handleSort = (column, sortDirection) => {
		...
		setDirection(sort);
		dispatch(assetsActions.getList(getUserData(), 1, perPageSelect == 0 ? perPage : perPageSelect, sort, filters ? filters: {}));
	};

	//Cambiar cantidad de filas
	const handlePerRowsChange = async (newPerPage, page) => {
		setPerPageSelect(newPerPage);
		dispatch(assetsActions.getList(getUserData(), page, newPerPage, direction, filters ? filters: {}));
    };
	...
	const onFilterData = (data, e) => {
		...
		setFilters(data);
		dispatch(assetsActions.getList(getUserData(), 1, perPageSelect == 0 ? perPage : perPageSelect, direction, data));
	};
    ...
    const deletingAssets = useSelector(state => state.assets.deleting);

	const deleteAsset = (data) => {
		dispatch(assetsActions.deleteAssets(data._id, user));
	}
	...
	const exportExcel = () => {
		...
		dispatch(assetsActions.getList(getUserData(), 1, perPageSelect == 0 ? perPage : perPageSelect, direction, filters, true));
	}
	...
	const exportToExcel = (data, filename = 'ListaDeActivos.xlsx') => {
		const worksheet = XLSX.utils.json_to_sheet(data);
		const workbook = XLSX.utils.book_new();

		XLSX.utils.book_append_sheet(workbook, worksheet, 'Lista De Activos');
					...
					<div className="flex-column flex-md-row p-3">
						...
						<div className="d-flex justify-content-between" style={{padding:"4px 16px 4px 24px"}}>
							<div className="align-self-center">
								<h3 style={{ fontWeight:'bold',fontStyle: 'italic',  marginBottom: '0'}}>Listado de Activos</h3>
							</div>
							{(user.role===1 || user.role == 9)&&<div>
							<span style={{ fontWeight: 'bold', marginRight: 8 }}>
								Añadir
							</span>
							<Button id="add" onClick={()=>history.push('/register-assets')} className="btn-round btn-icon" color="primary">
							...
							(user.role === Role.Admin)  &&  
								<>
									<div className="align-self-right">
											<h4 style={{ fontWeight:'bold',fontStyle: 'italic',  textAlign: 'right', marginRight: '100px'}}>Valor total: <NumberFormat value={totalAmount ? totalAmount.toFixed(2) : 0} displayType={'text'} thousandSeparator={','} decimalSeparator={'.'} prefix={'$ '} /></h4>
									</div>
								</>
						}
						<Row>
						...
						{/* Modal de confirmación de eliminación del activo */}
						<Modal toggle={() => { cleanDeleteData() }} isOpen={deleteModalOpen} className={`${darkMode ? "dark-mode" : ""}`}>
							...
							<div className={`modal-body ${darkMode ? "bg-dark text-white" : ""}`}>
								<p>Desea eliminar el activo { dataToDelete ? dataToDelete.name : ''} de la agencia {dataToDelete ? dataToDelete.agency.name : ''}</p>
								
								<div className="d-flex justify-content-between">
									<Button color="primary" disabled={deletingAssets} onClick={() => {deleteAsset(dataToDelete);}} >
										{deletingAssets && <span className="spinner-border spinner-border-sm mr-1"></span>}
