...
import { noteMarketActions, agencyActions } from '../../actions';
...
function SalesNotesListPage() {
    ...
    const noteMarketTable = useSelector(state => state.noteMarket.table);
    const loadingPage = useSelector(state => state.noteMarket.loading);
    ...
    // Función para manejar edición - navegar a SalesUpdated
    const handleEditNote = (row) => {
        console.log('row', row);
        const noteId = row._id;
        if (noteId) {
            history.push('/sales-updated', { id: noteId });
        }
    };
    ...
    const clearFilters = () => {
        ...
        dispatch(noteMarketActions.dataTable(getUserData(), 1, 10000, { id: 'createdDate', desc: true }, defaultFilters));
    };
    ...
    useEffect(() => {
        ...
        if (noteMarketState.successUpdated && !noteMarketState.updating) {
            dispatch(noteMarketActions.dataTable(getUserData(), 1, 10000, { id: 'createdDate', desc: true }, filters));
        }
    }, [noteMarketState.successUpdated, noteMarketState.updating, filters]);
    ...
    useEffect(() => {
        ...
        const unlisten = history.listen((location) => {
            // Si regresamos a /sales-notes desde otra ruta, recargar datos
            if (previousPath !== '/sales-notes' && location.pathname === '/sales-notes') {
    ...
    const headers = [
        ...
        { label: 'Encargado', key: 'marketAttendant' },
    ...
    useEffect(() => {
        if (isExporting && noteMarketTable && noteMarketTable.results && noteMarketTable.results.length > 0) {
            // Copiar datos para exportar
            let fixedData = noteMarketTable.results.map((item) => {
        ...
        } else if (!isExporting && noteMarketTable && noteMarketTable.results) {
            // Si no estamos exportando, actualizar la tabla normal
            setData(noteMarketTable.results);
        }
    }, [noteMarketTable, isExporting]);
    ...
    const columns = [
        ...
        { name: 'Encargado', selector: 'marketAttendant', sortable: true, wrap: true },
        { name: 'Total $', selector: 'totalDollar', sortable: true, cell: (row) => <NumberFormat value={row.totalDollar} displayType={'text'} thousandSeparator={','} decimalSeparator={'.'} prefix={'$ '} /> },
        { name: 'Total Bs', selector: 'total', sortable: true, cell: (row) => <NumberFormat value={row.total.toFixed(2)} displayType={'text'} thousandSeparator={','} decimalSeparator={'.'} prefix={'Bs '} /> },
        ...
        { name: 'Fecha', selector: 'createdDate', sortable: true, cell: (row) => moment(row.createdDate).utc().format('YYYY-MM-DD HH:mm') },
        {
            ...
            cell: (row) => {
                ...
                if (!isToday && ['ACEPT', 'ACEPTADO', 'ACCEPTED', 'MODIFIED', 'MODIFICADO'].includes(statusUpper)) {
                    return null;
                }
                            ...
                            <tbody style={{ color: "black" }}>
                                {originalProducts.map((product, index) => (
                                    <tr key={`orig-${index}`}>
                                        ...
                                        <td>
                                            <NumberFormat
                                                value={formatValue(product.price, 2)}
                                                displayType={'text'}
                                                thousandSeparator={','}
                                                decimalSeparator={'.'}
                                            />
                                        ...
                                        <td>
                                            <NumberFormat
                                                value={formatValue(product.kg, 3)}
                                                displayType={'text'}
                                                thousandSeparator={','}
                                                decimalSeparator={'.'}
                                            />
                                        ...
                                        <td>
                                            <NumberFormat
                                                value={formatValue(product.units, 0)}
                                                displayType={'text'}
                                                thousandSeparator={','}
                                                decimalSeparator={'.'}
                                            />
                                    ...
                                    .map((item, index) => {
                                        ...
                                        return (
                                            <tr key={`mod-before-${index}`}>
                                                ...
                                                <td>
                                                    <NumberFormat
                                                        value={(priceValue * kgValue).toFixed(2)}
                                                        displayType={'text'}
                                                        thousandSeparator={','}
                                                        decimalSeparator={'.'}
                                                        prefix={'Bs '}
        ...
        <div id="page-content-wrapper">
                ...
                <div className="flex-column flex-md-row p-3">
                    <div className="d-flex justify-content-between" style={{ padding: '4px 16px 4px 24px' }}>
                        <h3 style={{ marginBottom: '0', fontWeight: 'bold', fontStyle: 'italic' }}>Lista de notas de entrega</h3>
                    </div>
                    {/* Filtros */}
