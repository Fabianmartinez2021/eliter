...
function InventoryMiscellaneousHistoryPage() {
	...
	console.log('dataInventories', dataInventories);
	...
	const columns = [
        ...
        
        {
			...
			cell : (row)=>{
				return  <NumberFormat 
							value={row.in?row.in:row.in} 
							displayType={'text'} 
							thousandSeparator={','} 
							decimalSeparator={'.'}  
							decimalScale={row.in % 1 !== 0 ? 3 : 0}
                        	fixedDecimalScale={row.in % 1 !== 0}
						/>
			},
		},
		{
			...
			cell: (row) => {
				...
				return (
					...
					>
						<i className="fa fa-exclamation-circle text-yellow"></i>
					</Button>
					<NumberFormat 
						value={row.out ? row.out : row.out} 
						displayType={'text'} 
						thousandSeparator={','} 
						decimalSeparator={'.'}
						decimalScale={row.out % 1 !== 0 ? 3 : 0}
                        fixedDecimalScale={row.out % 1 !== 0} 
					/>
					</>
				);
				} else if (row.type === 3) {
				...
				return (
					...
					<Button 
						...
						onClick={e => e.preventDefault()}
					>
						<i className="fa fa-exclamation-circle text-green"></i>
					</Button>
					<NumberFormat 
						value={row.out ? row.out : row.out} 
						displayType={'text'} 
		...
		},
		{
			...
			cell: (row) => {
				let usedValue = 0;
				if (row.type === 14) {
					usedValue =  row.amountBefore - row.amountAfter ;
				}
				...
				return (
					<NumberFormat 
						...
						thousandSeparator={','} 
						decimalSeparator={'.'}
						decimalScale={usedValue % 1 !== 0 ? 3 : 0}
						fixedDecimalScale={usedValue % 1 !== 0}
					/>
				);
			},
		},
        {
			name: 'Queda',
			selector: 'amountAfter',
			sortable: true,
			cell : (row)=>	{
				return  <NumberFormat 
							value={row.amountAfter ? row.amountAfter: row.amountAfter} 
							displayType={'text'} 
							thousandSeparator={','} 
							decimalSeparator={'.'}
							decimalScale={row.amountAfter % 1 !== 0 ? 3 : 0}
							fixedDecimalScale={ row.amountAfter % 1 !== 0}
						/>
			},
        },
		{
	...
	const headers = [
		...
		{ label: "Nota", key: "note" },
		{ label: "Inicial", key: "amountBefore" },
		{ label: "Entrada", key: "in" },
		{ label: "Salida", key: "out" },
		{ label: "Queda", key: "amountAfter" }
	];
	...
	const getDataTable = (page) => {
		dispatch(miscellaneousInventoryActions.dataTableHistory(getUserData(), page, perPageSelect == 0 ? perPage : perPageSelect, direction, {}, false));
	}

	//Paginar
	const handlePageChange = async (page) => {
		dispatch(miscellaneousInventoryActions.dataTableHistory(getUserData(), page, perPageSelect == 0 ? perPage : perPageSelect, direction, filters ? filters: {}, false));
	};
	...
	const handleSort = (column, sortDirection) => {
		...
		setDirection(sort);
		dispatch(miscellaneousInventoryActions.dataTableHistory(getUserData(), 1, perPageSelect == 0 ? perPage : perPageSelect, sort, filters ? filters: {}, false));
	};

	//Cambiar cantidad de filas
	const handlePerRowsChange = async (newPerPage, page) => {
		setPerPageSelect(newPerPage);
		dispatch(miscellaneousInventoryActions.dataTableHistory(getUserData(), page, newPerPage, direction, filters ? filters: {}, false));
	};
	...
	const onFilterData = (data, e) => {
		...
		setFilters(data);
		dispatch(miscellaneousInventoryActions.dataTableHistory(getUserData(), 1, perPageSelect == 0 ? perPage : perPageSelect, direction, data, false));
	}
	...
	const exportExcel = () => {
		...
		dispatch(miscellaneousInventoryActions.dataTableHistory(getUserData(), 1, perPageSelect == 0 ? perPage : perPageSelect, direction, filters, true));
	}
	...
	const exportToExcel = (data, filename = 'Historialinvesumin.xlsx') => {
		const worksheet = XLSX.utils.json_to_sheet(data);
		const workbook = XLSX.utils.book_new();

		XLSX.utils.book_append_sheet(workbook, worksheet, 'Historial inven sumin');
	...
	useEffect(() => {
		if(excel && excel.results){
			...
			fixedData.forEach((item) => {
				...
				item.amountBefore = item.amountBefore.toString()
										.replace(/\,/g, '')  // se eliminan las comas
										.replace(".", ',');  // se cambia la coma por punto

				item.amountAfter = item.amountAfter.toString()
										.replace(/\,/g, '')  // se eliminan las comas
										.replace(".", ',');  // se cambia la coma por punto
...
export default InventoryMiscellaneousHistoryPage;