...
import { offlineFiscalActions } from '../../actions/offlineFiscal.actions'
...
function SalesFiscalCreateOfflinePage() {
    ...
    const pending = useSelector(state => state.pendingFiscal);

    //Cantidad de ventas offline pendientes por procesar
    const [pendingSalesFiscal, setPendingSalesFiscal] = useState('');

    const getPendings = () => {
        if (pending.salesFiscal && pending.salesFiscal.length > 0) {
            setPendingSalesFiscal(`(${pending.salesFiscal.length})`);
        } else {
            setPendingSalesFiscal("");
        }
    };
    ...
    const onProcessData = () => {
        ...
        if (pending.salesFiscal && pending.salesFiscal.length > 0) {
            let sales = {
                items: pending.salesFiscal,
            };

            dispatch(salesFiscalActions.processSalesFiscalOffline(sales));
        } else {
            console.log("onProcessData: No hay ventas pendientes para procesar.");
        }
    ...
    const registering = useSelector(state => state.offlineFiscal.registering);

    //State de registro offline para procesar todas las ventas
    const registeringOffline = useSelector(state => state.salesFiscal.registeringOfflineFiscal);
    ...
    // useEffect(() => {
    //     if (dataSale) {
    //         // Establecer los demás estados iniciales
    //         setListProducts(dataSale.products);
    //         setListSellers(dataSale.sellers);
    //         setListOperators(dataSale.operators);
    //         setTerminalList(dataSale.agency.terminal);
    ...
    //         // Buscar y mantener el valor de Pesos en la lista de monedas original
    //         const eurosCoin = dataSale.coins?.find(coin => coin.name == "Pesos");

    //         const pesosCoin = dataSale.coins?.find(coin => coin.name === "Euro");
    ...
    const onRegisterSale = (data, e) => {
        ...
        if (exceeded > 0) {
            ...
            dispatch(offlineFiscalActions.addSaleFiscalOffline(data));
        }
    ...
    const resetScreen = () => {
        resetSale({
            ...
            phone: watch("phone") || "0",
            operator: '',
            isTelesale: '', ves: '', dollar: '', eur: '', cop: '', tAmmount: '', tBank: '', tReference: '', pAmmount: '', pAmmountExtra: '', terminalExtra: '', pBank: '', pReference: '', pReferenceExtra: ''
        });
        setTotal(0);
        setTotalWeight(0);
        setTableSale([]);
    ...
    const statusRegister = useSelector(state => state.offlineFiscal);
                        ...
                        <Container>
                            <Row>
                                <Col sm="12" md={{ size: 8, offset: 2 }}>
                                    ...
                                    <div className="d-flex justify-content-between" style={{ marginBottom: 10 }}>
                                        <h3 style={{ fontWeight: 'bold', fontStyle: 'italic', marginBottom: '0' }}>Registro de venta especial offline al Detal </h3>
                                    </div>
                                    {alert.message &&
                                            ...
                                            {tableSale && tableSale.map((product, index) => {
                                                return (
                                                    <tr key={index}>
                                                        ...
                                                        <td>
                                                            <Button className="btn-link" color="info" style={{ margin: 0, padding: 0 }}
                                    ...
                                    <Form onSubmit={handleSubmitSale(onRegisterSale)}>
                                        ...
                                        <Row>
                                            <Col>
                                                <Button color="info" disabled={registering || registeringOffline}>
                                        ...
                                        <Row>
                                            <Col>
                                                <div className="pull-left">
                                                    <Button color="info" disabled={(!pending.salesFiscal || (pending.salesFiscal.length <= 0) || registering || registeringOffline)} onClick={e => { e.preventDefault(); onProcessData(); }}>
                                                        {registeringOffline && <span className="spinner-border spinner-border-sm mr-1"></span>}
                                                        Procesar Ventas {pendingSalesFiscal !== '' ? pendingSalesFiscal : ''}
                                                    </Button>
                                                </div>
                                            </Col>
                                        </Row>
...
export default SalesFiscalCreateOfflinePage;