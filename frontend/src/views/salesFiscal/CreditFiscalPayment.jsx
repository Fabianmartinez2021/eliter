...
function CreditFiscalPaymentPage() {
  ...
  const sales = useSelector(state => state.salesFiscal); //  Informacion de las ventas al Detal


  
  //Obtener monedas, productos y terminales de sucursal
  useEffect(() => {
    dispatch(salesFiscalActions.salesFiscalDataForm(user.agency.id));
  }, []);
  ...
  //State de guardado
  const registering = useSelector(state => state.salesFiscal.registering);
  const updatingPendigPayment = useSelector(state => state.pendingPaymentsFiscal.updating)
  ...
  const onRegisterSale = (dataIncomplete, e) => {
    ...
    if (!dataIncomplete || typeof dataIncomplete !== 'object') {
      setModalMsg('Error interno: datos de crédito no válidos.');
    ...
    if (!watchedData || typeof watchedData !== 'object') {
      setModalMsg('Error interno: datos del formulario no válidos.');
      setModalVisible(true);
      return;
    }

    // Continuar con datos seguros
    let data = { ...dataIncomplete, ...watchedData };
      ...
      data.type = 7 // ver sales.enum.js en el backend
      ...
      data.baseImponible= watch("baseImponible");
      data.exento= watch("exento");
      data.iva= watch("iva");
      ...
      if(exceeded > 0){
          ...
          dispatch(pendingPaymentsFiscalActions.pendingPaymentsUpdate( pendingPaymentOrder, data ));
      }
  ...
  const resetScreen = () => {
    resetSale({ documentType: '', document: '', names: '', businessName: '', clientType: '', taxpayer: '', phone: '', address: '',  ves: '', dollar: '', eur: '', cop: '', tAmmount: '', tBank: '', tReference: '', pAmmount: '', pAmmountExtra: '', terminalExtra: '', pBank: '', pReference: '', pReferenceExtra: '', baseImponible:'', iva:'', exento:'' });
    setSeller([])
    setTotal(0);
  ...
  const statusRegister = useSelector(state => state.pendingPaymentsFiscal);
  ...
  const onRegisterChange = (data, e) => {
    ...
    // Se modifica el arreglo para que sea similar al del cambio por abonos para que se pueda utilizar correctamente
    infoSale.changeDataPayment = { typeChangePayment: data.typeChange, changeAmmountPayment: data.changeAmmount }
    dispatch(pendingPaymentsFiscalActions.pendingPaymentsUpdate( pendingPaymentOrder, infoSale));
  }
  ...
  const dataPendingPayments = useSelector(state => state.pendingPaymentsFiscal.table);
  const loadingPendingPayments = useSelector(state => state.pendingPaymentsFiscal.loading);
  ...
  //obtener data de usuario necesaria
	...
	const getDataTable = (page) => {
		dispatch(pendingPaymentsFiscalActions.dataTable(getUserData(), page, perPageSelect == 0 ? perPage : perPageSelect, direction, {agency: user.agency.id, unpaid: true}));
	}
	...
	const handleSort = (column, sortDirection) => {
		...
		setDirection(sort);
		dispatch(pendingPaymentsFiscalActions.dataTable(getUserData(), 1, perPageSelect == 0 ? perPage : perPageSelect, sort, {agency: user.agency.id, unpaid: true}));
	};

	//Cambiar cantidad de filas
	const handlePerRowsChange = async (newPerPage, page) => {
		setPerPageSelect(newPerPage);
		dispatch(pendingPaymentsFiscalActions.dataTable(getUserData(), page, newPerPage, direction, {agency: user.agency.id, unpaid: true}));
	};
  ...
  const onClickPendingPayment = (row, event) => {
    ...
    setValue('baseImponible', row.baseImponible);
    setValue('exento', row.exento);
    setValue('iva', row.iva);
    ...
    setTotal(newTotal);
    setTotalCreditAmmount(row?.total ?? 0);
            ...
            <Container>
              <Row>
                <Col sm="12" md={{ size: 8, offset: 2 }}>
                  ...
                  <div className="d-flex justify-content-between" style={{ marginBottom: 10 }}>
                    <h3 style={{ fontWeight: 'bold', fontStyle: 'italic', marginBottom: '0' }}>Pagar o abonar crédito especial</h3>
                    <Button color="info" size="sm" onClick={() => openModalPendingPayments()}>
                  ...
                  <Row form>
                    ...
                    <Col md={1.5} style={{ display: "none" }}>
                      <FormGroup>
                        <Label for="baseImponible">Base Imponible</Label>
                        <input
                          name="baseImponible"
                          disabled = {true}
                          className={'form-control' + (errorsSale.baseImponible ? ' is-invalid' : '')}
                          ref={registerSale({ required: true })}
                        />
                        {errorsSale.baseImponible && <div className="invalid-feedback d-block">{errorsSale.baseImponible.message}</div>}
                      </FormGroup>
                    </Col>
                    <Col md={1.5} style={{ display: "none" }}>
                      <FormGroup>
                        ...
                        <input
                          name="exento"
                          disabled = {true}
                          className={'form-control' + (errorsSale.exento ? ' is-invalid' : '')}
                          ref={registerSale({ required: true })}
                        />
                        {errorsSale.exento && <div className="invalid-feedback d-block">{errorsSale.exento.message}</div>}
                      </FormGroup>
                    </Col>
                    <Col md={1.5} style={{ display: "none" }}>
                      <FormGroup>
                        ...
                        <input
                          name="iva"
                          disabled = {true}
                          className={'form-control' + (errorsSale.iva ? ' is-invalid' : '')}
                          ref={registerSale({ required: true })}
                        />
                        {errorsSale.iva && <div className="invalid-feedback d-block">{errorsSale.iva.message}</div>}
                      </FormGroup>
                    </Col>
                    {/* <Col md={2}>
                      <FormGroup>
                  ...
                  <Table bordered className="w-50">
                    ...
                    <tbody>
                      <tr>
                        ...
                        <td className="text-end">
                          {exento?.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                        </td>
                      </tr>
                    </tbody>
                  </Table>
                  ...
                  <Form onSubmit={handleSubmitSale(onRegisterSale)}>
                    ...
                    <Collapse isOpen={collapses.includes(1)}>
                      <div className="d-flex justify-content-between">
                        ...
                        {collapses.includes(1) ? <div className="pull-right align-self-center">
                          ...
                          {exceeded > 0 &&
                            <div color="info"> <i className="fa fa-exclamation-circle text-warning" aria-hidden="true"></i>{' '}
                    ...
                    <Row>
                      <Col>
                        <Button color="info" disabled={total == 0 || registering || updatingPendigPayment}>
                          {(updatingPendigPayment) && <span className="spinner-border spinner-border-sm mr-1"></span>}
                          Pagar crédito
                        </Button>{' '}
                      </Col>
                      <Col>
                        <Button color="info" disabled={total == 0 || registering || updatingPendigPayment || exceeded > 0 || !collapses.includes(1)} onClick={() => {onRegisterPayment(watch())}}>
                          {(updatingPendigPayment) && <span className="spinner-border spinner-border-sm mr-1"></span>}
                          Realizar Abono
                        </Button>{' '}
                      </Col>
                  ...
                  <div style={{ marginBottom: 20 }}>
                    {((exceeded > 0) || (modalChangePayment)) && <>
                      ...
                      <Form onSubmit={handleSubmitChange(onRegisterChange)}>
                        ...
                        <Row>
                          <Col>
                            <Button color="info" disabled={(registering || updatingPendigPayment)}>
...
export default CreditFiscalPaymentPage;