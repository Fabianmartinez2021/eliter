...
function InventoryAdjustmentHistoryPage() {
	...
	const headers = [
		...
		{ label: "Comentarios", key: "comment" },
		{ label: "Cantidad", key: "kg" }
	];
	...
	const getDataTable = (page) => {
		dispatch(inventoryActions.dataTableAdjustmentHistory(getUserData(), page, perPageSelect == 0 ? perPage : perPageSelect, direction, {}, false));
	}

	//Paginar
	const handlePageChange = async (page) => {
		dispatch(inventoryActions.dataTableAdjustmentHistory(getUserData(), page, perPageSelect == 0 ? perPage : perPageSelect, direction, filters ? filters: {}, false));
	};
	...
	const handleSort = (column, sortDirection) => {
		...
		setDirection(sort);
		dispatch(inventoryActions.dataTableAdjustmentHistory(getUserData(), 1, perPageSelect == 0 ? perPage : perPageSelect, sort, filters ? filters: {}, false));
	};

	//Cambiar cantidad de filas
	const handlePerRowsChange = async (newPerPage, page) => {
		setPerPageSelect(newPerPage);
		dispatch(inventoryActions.dataTableAdjustmentHistory(getUserData(), page, newPerPage, direction, filters ? filters: {}, false));
	};
	...
	const onFilterData = (data, e) => {
		...
		setFilters(data);
		dispatch(inventoryActions.dataTableAdjustmentHistory(getUserData(), 1, perPageSelect == 0 ? perPage : perPageSelect, direction, data, false));
	}
	...
	const exportExcel = () => {
		...
		dispatch(inventoryActions.dataTableAdjustmentHistory(getUserData(), 1, perPageSelect == 0 ? perPage : perPageSelect, direction, filters, true));
	}
	...
	const exportToExcel = (data, filename = 'Historialdeajustes.xlsx') => {
					...
					<div className="flex-column flex-md-row p-3">
						...
						<div className="d-flex justify-content-between" style={{padding:"4px 16px 4px 24px"}}>
							<div className="align-self-center">
								<h3 style={{ fontWeight:'bold',fontStyle: 'italic',  marginBottom: '0'}}>Historial de ajustes de inventario</h3>
							</div>
						</div>
						{/* Filtros */}
									...
									<FormGroup className="mr-3">
										<label>
											<input 
												...
												name="finalAdjustments"
												id="finalAdjustments" 
												type="checkbox" 
												value={true}
												ref={register}/> SÓLO AJUSTES FINALES
										</label>
									</FormGroup>
...
export default InventoryAdjustmentHistoryPage;