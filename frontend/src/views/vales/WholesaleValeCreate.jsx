...
import { agencyActions, salesActions, ticketActions, userActions } from '../../actions';
...
function WholevaleCreatePage() {
  ...
  const valueDollarRate =
    Number(dolarCoin?.value ?? listCoin?.[0]?.value ?? 0) || 0;
  const valueEurRate =
    Number(eurCoin?.value ?? listCoin?.[1]?.value ?? 0) || 0;
  const valueCopRate =
    Number(copCoin?.value ?? listCoin?.[2]?.value ?? 0) || 0;
  ...
  const onCreateData = (data, e) => {
    ...
    if (productFilter.length == 0) {
      ...
      const isCombo = codeStr.length > 0 && /^[Cc]/.test(codeStr[0]);
      ...
      //  Se verifica  que los kilogramos sean los minimos aceptados para una compra al mayor del producto (no aplica a combos — precio al detal)
      if (!isCombo && parseFloat(data.kg) < p.minWeight) {
        setModalVisible(true);
        const message = 'El peso del ' + p.name + ' debe ser mayor a ' + p.minWeight + ' ' + p.presentation;
        setModalMsg(message);
        return
      }

      //  Como los precios estan en dolares, se obtiene el valor de este para hacer las conversiones
      const valueDollar = parseFloat(valueDollarRate.toFixed(2));
      ...
      if (isCombo) {
        ...
        if (!priceUsd || priceUsd <= 0) {
          setModalVisible(true);
          setModalMsg('El combo no tiene un precio válido');
          return;
        }
        const bsUnit = parseFloat((priceUsd * valueDollar).toFixed(2));
        source = {
          kg: parseFloat(data.kg),
          price: bsUnit.toFixed(2),
          regularPrice: p.price * (listCoin[0]?.value),
          wholesalePrice: priceUsd,
          wholesalePriceBs: bsUnit,
          isWholesale: true,
          isOffer: false,
          total: parseFloat(data.kg) * bsUnit,
          totalDollars: parseFloat(data.kg) * priceUsd,
          applyWholesaleDiscount: false,
          wholesaleDiscountPrice: 0,
          wholesaleDiscountPriceBs: 0,
        };
      } else {
        //  Precio en dolares (mayor)
        let priceProductDollars = p.applyWholesaleDiscount ? p.wholesaleDiscountPrice : p.wholesalePrice;
        let priceProduct = parseFloat(priceProductDollars);
        source = {
          kg: parseFloat(data.kg),
          price: priceProduct.toFixed(2),
          regularPrice: p.price * (listCoin[0]?.value),
          wholesalePrice: parseFloat(p.wholesalePrice),
          wholesalePriceBs: parseFloat(p.wholesalePrice),
          isWholesale: true,
          isOffer: false,
          ...
          wholesaleDiscountPriceBs: parseFloat(p.wholesaleDiscountPrice) * valueDollar
        };
      }
      ...
      preSale.map((product) => {
        ...
        sumWithoutDiscount += parseFloat(product.kg) * parseFloat(product.wholesalePrice * valueDollar);
        sumDollarsWithoutDiscount += parseFloat(product.kg) * parseFloat(product.wholesalePrice);
  ...
  const onRegisterSale = (data, e) => {
    ...
    if (rowSelected && rowSelected.valueDollar != null) {
      ...
      data.valueEur = rowSelected.valueEur != null ? Number(rowSelected.valueEur).toFixed(2) : valueEurRate.toFixed(2);
      data.valueCop = rowSelected.valueCop != null ? Number(rowSelected.valueCop).toFixed(2) : valueCopRate.toFixed(2);
    } else {
      data.valueDollar = valueDollarRate.toFixed(2);
      data.valueEur = valueEurRate.toFixed(2);
      data.valueCop = valueCopRate.toFixed(2);
    }

    //  Ya que es una venta al mayor
    data.isWholesale = true;
    data.type = 13 // ver sales.enum.js en el backend
  ...
  const onRegisterCredit = async (data) => {
    ...
    if ((!data.documentType) || (!data.document) || (!data.names) || (!data.clientType) || (!data.phone) || (!data.address) || (!data.autorize)) {
      setModalVisible(true);
    ...
    //  Tipo de venta: 10 = Venta al mayor (ver sales.enum.js en el backend)
    //  Se establece al final para asegurar que no se sobrescriba
    data.type = 13;
  ...
  useEffect(() => {
    if (total > 0 && listCoin && listCoin.length > 0) {
      setTotalDollar(valueDollarRate > 0 ? total / valueDollarRate : 0);
      setTotalVes(total);
      setTotalEur(valueEurRate > 0 ? total / valueEurRate : 0);
      setTotalCop(valueDollarRate > 0 ? (total / valueDollarRate) * valueCopRate : 0);
    } else {
      setTotalDollar(0);
      setTotalVes(0);
      setTotalEur(0);
  ...
  useEffect(() => {
    if (rowSelected) {
      ...
      let valueDollar = valueDollarRate;
  ...
  useEffect(() => {
    if (exceeded > 0 && listCoin && listCoin.length > 0) {
      ...
      setTotalDollarChange(valueDollarRate > 0 ? exceeded / valueDollarRate : 0);
      setTotalEurChange(valueEurRate > 0 ? exceeded / valueEurRate : 0);
      setTotalCopChange(valueDollarRate > 0 ? (exceeded / valueDollarRate) * valueCopRate : 0);
    } else {
      setTotalBsChange(0);
  ...
  const handleSearch = (query) => {
    setIsLoading(true);
    setOptions([]);
    dispatch(userActions.getListValeClientTypeahead(query));
  };

  const handleSearchClientCode = (query) => {
    setIsLoading(true);
    setOptions([]);
    dispatch(userActions.getListValeClientByCodeTypeahead(query));
  };
            ...
            <Container>
              <Row>
                <Col sm="12" md={{ size: 8, offset: 2 }}>
                      ...
                      <div className="d-flex justify-content-between" style={{ marginBottom: 10 }}>
                        {listCoin[0] && (
                          <div style={{ fontSize: '0.9em' }}>
                            Dólar a {' '}
                            <b><NumberFormat value={valueDollarRate.toFixed(2)} displayType={'text'} thousandSeparator={true} prefix={'Bs '} /></b>
                          </div>
                        )}
                        {valueEurRate > 0 && (
                          <div style={{ fontSize: '0.9em' }}>
                            Euro a {' '}
                            <b><NumberFormat value={valueEurRate.toFixed(2)} displayType={'text'} thousandSeparator={true} prefix={'Bs '} /></b>
                          </div>
                        )}
                        {valueCopRate > 0 && (
                          <div style={{ fontSize: '0.9em' }}>
                            Pesos a {' '}
                            <b><NumberFormat value={valueCopRate.toFixed(2)} displayType={'text'} thousandSeparator={true} prefix={'Bs '} /></b>
                          </div>
                        )}
                      </div>
                    )}
                  ...
                  <div className="d-flex justify-content-between" style={{ marginBottom: 10 }}>
                    <h3 style={{ fontWeight: 'bold', fontStyle: 'italic', marginBottom: '0' }}>Registro de vale al mayor</h3>
                    ...
                    <Col className="text-right" style={{ margin: 0 }}>
                      <div className="d-inline-flex" style={{ padding: '5px 0px 10px 0px' }}>
                        ...
                        <div className="text-center" style={{ border: '1px solid #00C853', padding: 4 }}>
                          <b style={{ fontSize: 25, marginLeft: 10, marginRight: 10 }}>Total: <NumberFormat value={totalWithoutDiscount.toFixed(2)} displayType={'text'} thousandSeparator={true} />{' '}Bs</b>
                        </div>
                        <div className="text-center" style={{ border: '1px solid #00C853', borderTopRightRadius: '25px', borderBottomRightRadius: '25px', padding: 4 }}>
                          <b style={{ fontSize: 25, marginLeft: 10, marginRight: 10 }}>Total: <NumberFormat value={totalDollarsWithoutDiscount.toFixed(2)} displayType={'text'} thousandSeparator={true} />{' '}$</b>
                        </div>
                      </div>
                    </Col>
                  </Row>
                      ...
                      <div className="d-flex justify-content-between">
                        ...
                        {collapses.includes(1) ? <div className="pull-right align-self-center">
                          ...
                          <div>
                            ...
                            <b style={{ fontSize: 18, color: '#BC0707' }}> <NumberFormat value={(valueDollarRate > 0 ? (optionRest / valueDollarRate) : 0).toFixed(2)} displayType={'text'} thousandSeparator={true} prefix='$:    ' /> /  </b>
                            <b style={{ fontSize: 18, color: '#BC0707' }}> <NumberFormat value={(valueDollarRate > 0 ? (optionRest / valueDollarRate) * valueCopRate : 0).toFixed(2)} displayType={'text'} thousandSeparator={true} prefix='Cop:  ' /></b>
                          </div>
                          <br></br>
                          ...
                          <div>
                            ...
                            <b style={{ fontSize: 18 }}> <NumberFormat value={(valueDollarRate > 0 ? (optionTotal / valueDollarRate) : 0).toFixed(2)} displayType={'text'} thousandSeparator={true} prefix='$:    ' /> /  </b>
                            <b style={{ fontSize: 18 }}> <NumberFormat value={(valueDollarRate > 0 ? (optionTotal / valueDollarRate) * valueCopRate : 0).toFixed(2)} displayType={'text'} thousandSeparator={true} prefix='Cop:  ' /></b>
                          </div>
                          ...
                          {exceeded > 0 &&
                            ...
                            <div color="primary"> <i className="fa fa-exclamation-circle text-warning" aria-hidden="true"></i>{' '}
                              ...
                              <b style={{ fontSize: 18 }} className="text-warning"> <NumberFormat value={(valueDollarRate > 0 ? (exceeded / valueDollarRate) : 0).toFixed(2)} displayType={'text'} thousandSeparator={true} prefix='$  ' /> /  </b>
                              <b style={{ fontSize: 18 }} className="text-warning"> <NumberFormat value={(valueDollarRate > 0 ? (exceeded / valueDollarRate) * valueCopRate : 0).toFixed(2)} displayType={'text'} thousandSeparator={true} prefix='Cop  ' />   </b>
                            </div>
                      ...
                      <div className="form-row">
                        ...
                        <FormGroup className="col-md-4">
                          ...
                          >
                            ...
                            {/* <option key="FATTORIA BICENTENARIO" name="FATTORIA BICENTENARIO" value="EMBUTIDOS FATTORIA">EMBUTIDOS FATTORIA</option> */}
...
export default WholevaleCreatePage;