...
function PendingValesPage() {
    ...
    const dataPendingVales = useSelector(state => state.pendingVales.table);
    const loadingPage = useSelector(state => state.pendingVales.loading);
    ...
    useEffect(() => {
        ...
        if (dataPendingVales && dataPendingVales.results) {
            ...
            setData(dataPendingVales.results);
            setTotalPending(dataPendingVales.totalPending)
        }
    ...
    const columns = [

        {
            ...
            cell: row => {
                ...
                if (row.type === 12 || row.type === 13) {
                    if (row.comment && row.comment.trim().length > 0) {
                        label = row.status ? 'SALVADO' : 'PENDIENTE';

                    } else {
                        label = row.status ? 'PAGADO' : 'PENDIENTE';
    ...
    const headers = [
        ...
        { label: "Autorizado por", key: "autorize" },
    ...
    const getDataTable = (page) => {
        dispatch(pendingValesActions.dataTable(getUserData(), page, perPageSelect == 0 ? perPage : perPageSelect, direction, {}));
    }

    //Paginar
    const handlePageChange = async (page) => {
        dispatch(pendingValesActions.dataTable(getUserData(), page, perPageSelect == 0 ? perPage : perPageSelect, direction, filters ? filters : {}));
    };
    ...
    const handleSort = (column, sortDirection) => {
        ...
        setDirection(sort);
        dispatch(pendingValesActions.dataTable(getUserData(), 1, perPageSelect == 0 ? perPage : perPageSelect, sort, filters ? filters : {}));
    };

    //Cambiar cantidad de filas
    const handlePerRowsChange = async (newPerPage, page) => {
        setPerPageSelect(newPerPage);
        dispatch(pendingValesActions.dataTable(getUserData(), page, newPerPage, direction, filters ? filters : {}));
    };
    ...
    const onFilterData = (data, e) => {
        ...
        dispatch(pendingValesActions.dataTable(getUserData(), 1, perPageSelect == 0 ? perPage : perPageSelect, direction, data));
    };
                ...
                <tbody>
                    {data.products && data.products.map((product, index) => {
                        return (
                            <tr key={index}>
                                <td>{product.name}</td>
                                <td> {product.isWholesale ? '$' : '$'}{' '}<NumberFormat value={product.wholesalePrice.toFixed(2)} displayType={'text'} thousandSeparator={','} decimalSeparator={'.'} /></td>
                ...
                <Table striped responsive>
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td>{data.documentType + '-' + data.document}</td>
    ...
    const exportExcel = () => {
        ...
        dispatch(pendingValesActions.dataTable(getUserData(), 1, perPageSelect == 0 ? perPage : perPageSelect, direction, filters, true));
    }
    ...
    const exportToExcel = (data, filename = 'ValesPendientes.xlsx') => {
        const worksheet = XLSX.utils.json_to_sheet(data);
        const workbook = XLSX.utils.book_new();

        XLSX.utils.book_append_sheet(workbook, worksheet, 'Vales Pendientes');
    ...
    useEffect(() => {
        if (excel && excel.results) {
            ...
            let fixedData = excel.results.map((item) => ({ ...item }));

            fixedData.forEach((item) => {
                const formatNumber = (num) =>
                    num != null ? parseFloat(num).toFixed(2).replace(".", ",") : "0,00";

                item.pending = formatNumber(item.pending);
                item.tAmmount = formatNumber(item.tAmmount);
                item.pAmmount = formatNumber(item.pAmmount);
                item.dollar = formatNumber(item.dollar);
                item.ves = formatNumber(item.ves);
                item.cop = formatNumber(item.cop);
                item.valueDollar = formatNumber(item.valueDollar);
                ...
                <div id="page-content-wrapper">
                    ...
                    <div className="flex-column flex-md-row p-3">
                        ...
                        <div className="d-flex justify-content-between" style={{ padding: "4px 16px 4px 24px" }}>
                            <div className="align-self-center">
                                <h3 style={{ fontWeight: 'bold', fontStyle: 'italic', marginBottom: '0' }}>Vales por cobrar</h3>
                            </div>
                                <div className="d-flex justify-content-between flex-row">
                                    <div className='d-flex flex-row align-items-center'>
                                        ...
                                        <Button id="add" onClick={() => history.push('/register-vale')} className="btn-round btn-icon" color="primary">
                                            <i className="fa fa-plus" />
                                        </Button>
                                    </div>
                                    <div className='d-flex flex-row align-items-center'>
                                        <span style={{ fontWeight: 'bold', marginRight: 8, marginLeft: 8 }}>
                                            Pagar Vale
                                        </span>
                                        <Button id="add" onClick={() => history.push('/vale-payments')} className="btn-round btn-icon" color="primary">
...
export default PendingValesPage;