...
function PaymentFiscalMethodsHistory() {
    ...
    const dataTable = useSelector(state => state.salesFiscal.table);
    const loadingPage = useSelector(state => state.sales.loading);

    //Verificar data de redux
    useEffect(() => {

        if(dataTable && dataTable.resultsStores ){
    ...
    const columns = [
        ...
        },
        {
            ...
            sortable: true,
            cell : (row)=>{
                return moment(row.date).utc().format("YYYY-MM-DD hh:mm:ss a");
            },
        },
        {
            name: '',
            ...
            cell : (row)=>{
                return <>
                    <div className="d-flex justify-content-end align-items-center" style={{ fontWeight:'bold',fontStyle: 'italic',  textAlign: 'right'}}>
                        <Button color="info" disabled={loadingPage} onClick={() => {setDataToPrint(row)}} style={{
                            marginRight: '5px',
                            width: '100px', 
                            padding: '10px 10px', 
                        }}>
    ...
    const getDataTable = (page) => {
        dispatch(salesFiscalActions.salesFiscalPaymentMethodsHistory(getUserData(), page, perPageSelect == 0 ? perPage : perPageSelect, direction, {}, false));
    }

    //Paginar
    const handlePageChange = async (page) => {
        dispatch(salesFiscalActions.salesFiscalPaymentMethodsHistory(getUserData(), page, perPageSelect == 0 ? perPage : perPageSelect, direction, filters ? filters: {}, false));
    };
    ...
    const handleSort = (column, sortDirection) => {
        ...
        setDirection(sort);
        dispatch(salesFiscalActions.salesFiscalPaymentMethodsHistory(getUserData(), 1, perPageSelect == 0 ? perPage : perPageSelect, sort, filters ? filters: {}, false));
    };

    //Cambiar cantidad de filas
    const handlePerRowsChange = async (newPerPage, page) => {
        setPerPageSelect(newPerPage);
        dispatch(salesFiscalActions.salesFiscalPaymentMethodsHistory(getUserData(), page, newPerPage, direction, filters ? filters: {}, false));
    };
    ...
    const onFilterData = (data, e) => {
        ...
        setFilters(data);
        dispatch(salesFiscalActions.salesFiscalPaymentMethodsHistory(getUserData(), 1, perPageSelect == 0 ? perPage : perPageSelect, direction, data, false));
    }
            ...
            <Table responsive hover bordered size="sm">
                <tbody style={{textAlign:'center', color: "black"}}>
                    <tr style={{height:'7rem', fontSize: 'x-large'}}>
                        <th style={{verticalAlign: 'middle'}} rowSpan={1} colSpan={12}> Cierre del dia {moment(data.date).format('dddd')} {moment(data.date).format('LL')} <br></br> {data.agencyInfo.name}</th>
                    </tr>
                    <tr style={{height: "0"}}>
                   ...
                   <tr>
                        <td rowSpan={1} colSpan={3}><NumberFormat value={data.virtualValues.valueDollar.toFixed(2)} displayType={'text'} thousandSeparator={','} decimalSeparator={'.'} prefix={'Bs '} /></td>
                        <td rowSpan={1} colSpan={3}><NumberFormat value={data.virtualValues.valueCop.toFixed(2)} displayType={'text'} thousandSeparator={','} decimalSeparator={'.'} prefix={'Bs '} /></td>
                        <td rowSpan={1} colSpan={3}><NumberFormat value={data.virtualValues.valueEur.toFixed(2)} displayType={'text'} thousandSeparator={','} decimalSeparator={'.'} prefix={'Bs '} /></td>
                    </tr> */}
                    <tr>
                    ...
                    <tr>
                        ...
                        <td rowSpan={1} colSpan={6}><NumberFormat value={(data.virtualValues.totalAmount).toFixed(2)} displayType={'text'} thousandSeparator={','} decimalSeparator={'.'} prefix={'Bs '} /></td>

                    </tr>
                    <tr>
                        <th rowSpan={1} colSpan={6}>Diferencia</th>
                        <td rowSpan={1} colSpan={6}><NumberFormat value={(toNumber(data.total) - toNumber(data.virtualValues.totalAmount)).toFixed(2)} displayType={'text'} thousandSeparator={','} decimalSeparator={'.'} prefix={'Bs '} /></td>
                    </tr>
                     ...
                     {buttonStatus && <tr>
                             <td rowSpan={1} colSpan={12}>
                                 <div className="d-flex justify-content-center flex-row align-items-center" style={{ fontWeight:'bold',fontStyle: 'italic',  textAlign: 'center'}}>
                                     <Button color="info" disabled={loadingPage} onClick={() => {setDataToPrint(data)}}>
                                         <Icon icon={pdfIcon} /> PDF{loadingPage && <span className="spinner-border spinner-border-sm mr-1"></span>} 
                                     </Button>
                                     <Button color="info" disabled={loadingPage} onClick={() => {exportToExcel(data)}}>
                                         <Icon icon={excelIcon} /> Excel{loadingPage && <span className="spinner-border spinner-border-sm mr-1"></span>} 
                                     </Button>
                                 </div>
                             </td>
    ...
    const handlePrint = useReactToPrint({
        ...
        documentTitle: (dataToPrint ? dataToPrint.agencyInfo.name : '') + ' ' + (dataToPrint ? moment(dataToPrint.date).utc().format("YYYY-MM-DD") : ''),
    ...
    const exportToExcel = (data) => {
        ...
        const exportData = [
            ['Nombre de la Agencia', data.agencyInfo?.name || 'N/A'],
            ['Fecha', moment(data.date).utc().format("YYYY-MM-DD")],
            [''],
            ['Total Base Imponible', data?.baseImponible ? data.baseImponible.toFixed(2).replace(".", ',') : '0,00'],
            ['Total Base Imponible según sistema', data.virtualValues?.totalBaseImponible ? data.virtualValues.totalBaseImponible.toFixed(2).replace(".", ',') : '0,00'],
            ['Diferencia', (data?.baseImponible - data.virtualValues?.totalBaseImponible).toFixed(2).replace(".", ',')],
            ['Total Exento', data?.exento ? data.exento.toFixed(2).replace(".", ',') : '0,00'],
            ['Total Exento según sistema', data.virtualValues?.totalExento ? data.virtualValues.totalExento.toFixed(2).replace(".", ',') : '0,00'],
            ['Diferencia', (data?.exento - data.virtualValues?.totalExento).toFixed(2).replace(".", ',')],
            ['Total Base IVA', data?.IVA ? data.IVA.toFixed(2).replace(".", ',') : '0,00'],
            ['Total Base IVA según sistema', data.virtualValues?.totalIVA ? data.virtualValues.totalIVA.toFixed(2).replace(".", ',') : '0,00'],
            ['Diferencia', (data?.IVA - data.virtualValues?.totalIVA).toFixed(2).replace(".", ',')],
            ['Total', data?.total ? data.total.toFixed(2).replace(".", ',') : '0,00'],
            ['Total según sistema', data.virtualValues?.totalAmount ? data.virtualValues.totalAmount.toFixed(2).replace(".", ',') : '0,00'],
            ['Diferencia', (data?.total - data.virtualValues?.totalAmount).toFixed(2).replace(".", ',')],
        ];
        ...
        XLSX.writeFile(wb, `${data.agencyInfo?.name || 'Reporte'}_${moment(data.date).utc().format("YYYY-MM-DD")}.xlsx`);
    };
                    ...
                    <div className="flex-column flex-md-row p-3">
                        ...
                        <div className="d-flex justify-content-between" style={{padding:"4px 16px 4px 24px"}}>
                            <div className="align-self-center">
                                <h3 style={{ fontWeight:'bold',fontStyle: 'italic',  marginBottom: '0'}}>Historial de cierre Z</h3>
                            </div>
                        </div>
                        {/* Filtros */}
...
export default PaymentFiscalMethodsHistory;