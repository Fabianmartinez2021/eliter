...
function InventoryFiscalHistoryPage() {
	...
	const filteredData = user.role === 1 || user.role === 2 || user.role === 9 ?  data : data.filter(item => item.agency === user.agency.id);
	...
	const columns = [
        ...
        {
			name: 'Sucursal',
			selector: (row) => row.agencyDetails?.name || "",
			sortable: true,
			wrap:true,
        },
		{
			name: 'Cod. Producto',
			selector: (row) => row.productDetails?.code || "",
			sortable: true,
		},
		{
			name: 'Producto',
			selector: (row) => row.productDetails?.name || "",
			sortable: true,
			wrap:true,
		},
		{
			name: 'Tipo',
			selector: (row) =>
				row.productDetails?.exempt
					? "E"
					: row.productDetails?.taxed
					? "G"
					: "",
			sortable: true,
		},
	...
	const headers = [
		...
		{ label: "Sucursal", key: "agencyDetails.name" },
		{ label: "Código Producto", key: "productDetails.code" },
		{ label: "Producto", key: "productDetails.name" },
	...
	const getDataTable = (page) => {
		dispatch(inventoryFiscalActions.dataTableHistory(getUserData(), page, perPageSelect == 0 ? perPage : perPageSelect, direction, {}, false));
	}

	//Paginar
	const handlePageChange = async (page) => {
		dispatch(inventoryFiscalActions.dataTableHistory(getUserData(), page, perPageSelect == 0 ? perPage : perPageSelect, direction, filters ? filters: {}, false));
	};
	...
	const handleSort = (column, sortDirection) => {
		...
		setDirection(sort);
		dispatch(inventoryFiscalActions.dataTableHistory(getUserData(), 1, perPageSelect == 0 ? perPage : perPageSelect, sort, filters ? filters: {}, false));
	};

	//Cambiar cantidad de filas
	const handlePerRowsChange = async (newPerPage, page) => {
		setPerPageSelect(newPerPage);
		dispatch(inventoryFiscalActions.dataTableHistory(getUserData(), page, newPerPage, direction, filters ? filters: {}, false));
	};
	...
	const onFilterData = (data, e) => {
		...
		setFilters(data);
		dispatch(inventoryFiscalActions.dataTableHistory(getUserData(), 1, perPageSelect == 0 ? perPage : perPageSelect, direction, data, false));
	}
	...
	const exportExcel = () => {
		...
		dispatch(inventoryFiscalActions.dataTableHistory(getUserData(), 1, perPageSelect == 0 ? perPage : perPageSelect, direction, filters, true));
	}
	...
	const exportToExcel = (data, filename = 'historialespecial.xlsx') => {
		const worksheet = XLSX.utils.json_to_sheet(data);
		const workbook = XLSX.utils.book_new();

		XLSX.utils.book_append_sheet(workbook, worksheet, 'Historial especial');
					...
					<div className="flex-column flex-md-row p-3">
						...
						<div className="d-flex justify-content-between" style={{padding:"4px 16px 4px 24px"}}>
							<div className="align-self-center">
								<h3 style={{ fontWeight:'bold',fontStyle: 'italic',  marginBottom: '0'}}>Historial de inventario especial</h3>
							</div>
						</div>
						{/* Filtros */}
...
export default InventoryFiscalHistoryPage;