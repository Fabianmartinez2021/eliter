...
function AgencyCloseHistoryPage() {
	...
	const dataHistory = useSelector(state => state.agencies.history);
    const loadingPage = useSelector(state => state.agencies.loading);

	//Verificar data de redux
	useEffect(() => {
		if(dataHistory && dataHistory.results){
			setData(dataHistory.results);
		}
		if(dataHistory && dataHistory.metadata && dataHistory.metadata[0]){
			setRowCount(dataHistory.metadata[0].total);
		}
  	},[dataHistory]);
	...
	const getDataTable = (page) => {
		dispatch(agencyActions.agencyCloseHistory(getUserData(), page, perPageSelect == 0 ? perPage : perPageSelect, direction, {}, false));
	}

	//Paginar
	const handlePageChange = async (page) => {
		dispatch(agencyActions.agencyCloseHistory(getUserData(), page, perPageSelect == 0 ? perPage : perPageSelect, direction, filters ? filters: {}, false));
	};
	...
	const handleSort = (column, sortDirection) => {
		...
		setDirection(sort);
		dispatch(agencyActions.agencyCloseHistory(getUserData(), 1, perPageSelect == 0 ? perPage : perPageSelect, sort, filters ? filters: {}, false));
	};

	//Cambiar cantidad de filas
	const handlePerRowsChange = async (newPerPage, page) => {
		setPerPageSelect(newPerPage);
		dispatch(agencyActions.agencyCloseHistory(getUserData(), page, newPerPage, direction, filters ? filters: {}, false));
	};
	...
	const onFilterData = (data, e) => {
		...
		setFilters(data);
		dispatch(agencyActions.agencyCloseHistory(getUserData(), 1, perPageSelect == 0 ? perPage : perPageSelect, direction, data, false));
	}
	...
	const exportExcel = () => {
		...
		dispatch(agencyActions.agencyCloseHistory(getUserData(), 1, perPageSelect == 0 ? perPage : perPageSelect, direction, filters, true));
	}
	...
	const exportToExcel = (data, filename = 'HistorialCierreTiendas.xlsx') => {
		const worksheet = XLSX.utils.json_to_sheet(data);
		const workbook = XLSX.utils.book_new();

		XLSX.utils.book_append_sheet(workbook, worksheet, 'Historial Cierre de Tiendas');
					...
					<div className="flex-column flex-md-row p-3">
						...
						<div className="d-flex justify-content-between" style={{padding:"4px 16px 4px 24px"}}>
							<div className="align-self-center">
								<h3 style={{ fontWeight:'bold',fontStyle: 'italic',  marginBottom: '0'}}>Historial de cierre de tiendas</h3>
							</div>
						</div>
						{/* Filtros */}
						<div className="filter">
							...
							{isOpen && <>
								<Form onSubmit={handleSubmit(onFilterData)} className="form-inline" style={{marginTop:15}}>
                                    {(user.role == 1 || user.role == 2 || user.role == 10) && <FormGroup className="mr-3">
...
export default AgencyCloseHistoryPage;