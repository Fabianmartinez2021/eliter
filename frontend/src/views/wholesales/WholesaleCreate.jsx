...
import { salesActions, ticketActions, userActions, pendingPaymentsActions } from '../../actions';
...
function WholesaleCreatePage() {
  ...
  useEffect(() => {
    if (user && user.agency && user.agency.id) {
      const fetchWeeklyTotal = async () => {
        ...
        try {
          const data = await salesService.getWeeklyWholesalesGoalRealtime({ 
  ...
  const onCreateData = (data, e) => {
    ...
    if (productFilter.length == 0) {
      ...
      //  Precio en dolares
      let priceProductDollars = productFilter[0].applyWholesaleDiscount ? productFilter[0].wholesaleDiscountPrice : productFilter[0].wholesalePrice;
      ...
      const source = {
        ...
        wholesalePrice: parseFloat(productFilter[0].wholesalePrice),
  ...
  const onRegisterSale = (data, e) => {
    ...
    if (rowSelected && rowSelected.valueDollar != null && rowSelected.valueDollar !== undefined) {
      ...
      data.valueEur = (rowSelected.valueEur != null && rowSelected.valueEur !== undefined) ? Number(rowSelected.valueEur).toFixed(2) : listCoin[1].value.toFixed(2);
      data.valueCop = (rowSelected.valueCop != null && rowSelected.valueCop !== undefined) ? Number(rowSelected.valueCop).toFixed(2) : listCoin[2].value.toFixed(2);
    } else {
    ...
    data.type = 2 // ver sales.enum.js en el backend
  ...
  const onRegisterCredit = async (data) => {
    ...
    data.isCredit = true;
    data.type = 4 // ver sales.enum.js en el backend
  ...
  useEffect(() => {
    if (statusRegisterPendingPayments.success) {
      ...
      clientNamesRef.current.clear();
      clientNamesRef.current.focus();
      // Actualizar total semanal después de registrar crédito
      if (user && user.agency && user.agency.id) {
        const fetchWeeklyTotal = async () => {
          try {
            const data = await salesService.getWeeklyWholesalesGoalRealtime({ 
  ...
  const saveTicket = async () => {
    ...
    if (listCoin && listCoin.length >= 3) {
      ticket.valueDollar = parseFloat(listCoin[0].value);
      ticket.valueEur = parseFloat(listCoin[1].value);
      ticket.valueCop = parseFloat(listCoin[2].value);
    }
  ...
  // Booleano de meta alcanzada basado en wholesalesGoalReached
  const isGoalMet = weeklyTotal?.result?.wholesalesGoalCalculation?.wholesalesGoalReached || false;
  ...
  const goalKey = weeklyTotal?.result?.weekInfo?.startDate || null;

  // Al cambiar de semana/meta, leemos si ya se vio la animación esta semana
  useEffect(() => {
    if (!goalKey) return;

    const storageKey = `weeklyWholesalesGoalConfettiSeen_${goalKey}`;
      ...
      <div className={`d-flex ${darkMode ? "dark-mode" : ""}`} id="wrapper">
        ...
        <div id="page-content-wrapper" style={{ position: 'relative' }}>
            ...
            <Container>
              <Row>
                <Col sm="12" md={{ size: 8, offset: 2 }}>
                  ...
                  <div className="d-flex justify-content-between" style={{ marginBottom: 10 }}>
                    <h3 style={{ fontWeight: 'bold', fontStyle: 'italic', marginBottom: '0' }}>Registro de venta al mayor</h3>
                  ...
                  <Form onSubmit={handleSubmitSale(onRegisterSale)}>
                    ...
                    <Row>
                      <Col>
                        <Button color="primary" disabled={total == 0 || registering || registeringTicket || registeringPendingPayments || watch('isSolvent') === false}>
                          {registering && <span className="spinner-border spinner-border-sm mr-1"></span>}
                          Cobrar
                        </Button>{' '}
                        <Button color="primary" onClick={() => { saveTicket() }} disabled={total == 0 || registering || registeringTicket || registeringPendingPayments}>
                        ...
                        <Button color="primary" onClick={() => { onRegisterCredit(watch()) }} disabled={total == 0 || registering || registeringTicket || registeringPendingPayments || watch('isSolvent') === false}>
          ...
          <MetaBubble
            ...
            percentage={weeklyTotal?.wholesalesGoalCalculation?.wholesalesGoalPercentage}
            reached={weeklyTotal?.wholesalesGoalCalculation?.wholesalesGoalReached || false}
            loading={!weeklyTotal}
            darkMode={darkMode}
            isMobile={isMobile}
          />
...
export default WholesaleCreatePage;