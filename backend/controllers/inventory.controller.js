...
router.post('/report-inventories-summary-agency/', getDailyInventorySummaryByAgency);
...
/**
 * POST getTable - Tabla de inventario (Registrar inventario físico).
...
function getTable(req, res, next) {
    ...
    const ensureUser = !params.user && req.user
        ? userService.getById(req.user.sub).then((user) => {
            ...
            return inventoryService.dataTable(params);
        })
        : Promise.resolve(inventoryService.dataTable(params));
...
function reportSales(req, res, next) {
    inventoryService.reportSales(req.body)
        .then(inventories => inventories ? res.json(inventories) : res.status(400).json({ message: 'No hay información disponibe' }))
        .catch(err => next(err));
}
...
function getTableHistory(req, res, next) {
    inventoryService.dataTableHistory(req.body)
        .then(inventories => inventories ? res.json(inventories) : res.status(400).json({ message: 'No hay información disponibe' }))
        .catch(err => next(err));
}
...
function getInventoryReport(req, res, next) {
    inventoryService.getInventoryReport(req.body)
        .then(inventories => inventories ? res.json(inventories) : res.status(400).json({ message: 'No hay información disponibe' }))
        .catch(err => next(err));
}
 ...
 * Usa la misma lógica de getInventoryReport y retorna totales por sucursal.
 */
function getDailyInventorySummaryByAgency(req, res, next) {
    inventoryService.getDailyInventorySummaryByAgency(req.body)
        .then(summary => summary ? res.json(summary) : res.status(400).json({ message: 'No hay información disponibe' }))
        .catch(err => next(err));
}

/**
 * Reporte de balances - asegura recibir user (role/agency) desde token si no viene en body
 */
function getBalanceReport(req, res, next) {
    ...
    const ensureUser = !params.user && req.user
        ? userService.getById(req.user.sub).then((user) => {
            if (user) params.user = user;
            return inventoryService.getBalanceReport(params);
        })
        : Promise.resolve(inventoryService.getBalanceReport(params));
