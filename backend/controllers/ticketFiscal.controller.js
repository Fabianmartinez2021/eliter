...
const ticketFiscalService = require('../services/ticketFiscal.service');
...
function create(req, res, next) {
    ticketFiscalService.create(req.body)
...
function update(req, res, next) {
    ticketFiscalService.update(req.params.id, req.body)
        .then(() => res.json({}))
        .catch(err => next(err));
}
...
function getTable(req, res, next) {
    ticketFiscalService.dataTable(req.body)
 ...
 */
 function getTableWholesale(req, res, next) {
    ticketFiscalService.dataTableWholesale(req.body)
        .then(tickets => tickets ? res.json(tickets) : res.status(400).json({ message: 'No hay información disponibe' }))
        .catch(err => next(err));
}
...
function removeTicket(req, res, next) {
    ...
    userService.getById(req.user.sub)
        .then(user => ticketFiscalService.removeTicket(req.params.id, { ...req.body, user }))
        .then(tickets => tickets ? res.json(tickets) : res.status(400).json({ message: 'No hay información disponibe' }))
        .catch(err => next(err));
}