...
/**
 * Revierte movimientos de inventario registrados durante extendCreditItems.
 */
async function rollbackExtendInventoryRecords(dataToDelete, agency) {
    for (let recordID of dataToDelete.InventoryRecord) {
        let item = await InventoryRecord.findByIdAndDelete(recordID);
        if (!item) continue;
        let inventory = await Inventory.findOne({ product: item.product, agency }).populate('product');
        if (inventory) {
            let total = parseFloat(inventory.kg) + parseFloat(item.out);
            await Inventory.findOneAndUpdate({ product: item.product, agency }, { kg: total });
        }
    }
}

/**
 * Salida de inventario por ítem (misma lógica que create en pendingPayments).
 */
async function inventoryExitForCreditItem(item, agency, differential, dataToDelete) {
    const ch = String(item.code)[0];
    if (ch === 'D' || ch === 'd') {
        return;
    }

    let inventory = await Inventory.findOne({ product: item.id, agency }).populate('product');
    ...
    if (inventory) {
        ...
        inventoryParam.agency = agency;
        inventoryParam.kg = inventory.kg;
        inventoryParam.in = 0;
        inventoryParam.out = item.kg;
        inventoryParam.total = total;
        inventoryParam.note = ' ';
        inventoryParam.price = item.price;
        inventoryParam.regularPrice = item.regularPrice;
        inventoryParam.differential = differential;
        inventoryParam.isOffer = item.isOffer;
        inventoryParam.isWholesale = item.isWholesale ? true : false;
        ...
        await Inventory.findOneAndUpdate({ product: item.id, agency }, { kg: total });
        ...
        try {
            if (inventory.product && (inventory.product.decrease || inventory.product.mincemeat)) {
                ...
                await Inventory.findOneAndUpdate({ product: item.id, agency }, { kg: totalDecrease });
            }
        } catch (e) {
...
/**
 * Añade productos a un crédito pendiente (aumenta total y pendiente, descuenta inventario).
 */
async function extendCreditItemsImpl(id, dataParams) {
    if (dataParams.isTelesale === '') {
        dataParams.isTelesale = false;
    }
    if (!dataParams.items || !Array.isArray(dataParams.items) || dataParams.items.length === 0) {
        throw 'Debe indicar al menos un producto para añadir a la deuda';
    }

    const orderId = parseInt(id, 10);
    let pendingPayment = await PendingPayments.findOne({ order: orderId });

    if (!pendingPayment) {
        pendingPayment = await PendingPayments.findOne({
            document: dataParams.document,
            type: 8,
            status: false,
        }).sort({ createdDate: -1 });
    }
    ...
    if (!pendingPayment) {
        throw 'No se encontró el pago pendiente';
    }
    if (pendingPayment.status) {
        throw 'El crédito ya fue pagado';
    }
    if (String(pendingPayment.agency) !== String(dataParams.agency)) {
        throw 'La deuda no es de esta agencia';
    }

    const agency = dataParams.agency;
    const dataToDelete = { InventoryRecord: [] };

    try {
        let totalDifferential = 0;
        const newProductsForPending = [];

        for (let item of dataParams.items) {
            let differential =
                item.isOffer || item.isWholesale
                    ? parseFloat(item.kg) * item.regularPrice - parseFloat(item.kg) * item.wholesalePriceBs
                    : 0;
            if (item.code && (String(item.code)[0] === 'D' || String(item.code)[0] === 'd')) {
                differential = 0;
            }

            const code0 = item.code ? String(item.code)[0] : '';
            const tryCombo = code0 && code0 !== 'D' && code0 !== 'd';

            if (tryCombo) {
                try {
                    const combosDifferential = await salesService.processCombos(
                        { agency, isWholesale: false, isCredit: true },
                        item,
                        dataToDelete
                    );
                    totalDifferential += differential + combosDifferential;

                    newProductsForPending.push({
                        ...
                        totalDollars: parseFloat(item.totalDollars),
                    });
                    continue;
                } catch (err) {
            ...
            await inventoryExitForCreditItem(item, agency, differential, dataToDelete);
        }

        const addedDollars = dataParams.items.reduce((s, i) => s + parseFloat(i.totalDollars), 0);

        pendingPayment.products = pendingPayment.products.concat(newProductsForPending);
        pendingPayment.markModified('products');
        pendingPayment.total = parseFloat(pendingPayment.total) + addedDollars;
        pendingPayment.pending = parseFloat(pendingPayment.pending) + addedDollars;
        pendingPayment.differential = (parseFloat(pendingPayment.differential) || 0) + totalDifferential;

        if (dataParams.addedTotalWeight !== undefined && dataParams.addedTotalWeight !== null && dataParams.addedTotalWeight !== '') {
            const w = parseFloat(String(dataParams.addedTotalWeight).replace(/,/g, ''));
            if (!isNaN(w)) {
                pendingPayment.totalWeight = (parseFloat(pendingPayment.totalWeight) || 0) + w;
            }
        }

        if (dataParams.seller) {
            pendingPayment.seller = dataParams.seller;
        }
        if (dataParams.isTelesale !== undefined && dataParams.isTelesale !== null && dataParams.isTelesale !== '') {
            pendingPayment.isTelesale = dataParams.isTelesale === true || dataParams.isTelesale === 'true';
        }

        const saved = await pendingPayment.save();

        const client = await WholesaleClient.findOne({ document: dataParams.document });
        if (!client) {
            throw 'Cliente no encontrado';
        }
        client.pendingPayment = saved;
        client.totalSpent = (parseFloat(client.totalSpent) || 0) + addedDollars;
        await client.save();

        return saved;
    } catch (error) {
        await rollbackExtendInventoryRecords(dataToDelete, agency);
        throw error;
    }
}

let pendingPaymentsService = {
    ...
    create: async (dataParams) => {
        ...
        try {
            ...
            for (let item of dataParams.items) {
                ...
                // Combos (catálogo), igual que venta al detalle / extendCreditItems
            ...
            // Crédito desde venta al detalle (u otros flujos) sin datos fiscales completos
            if (!dataParams.documentType || !String(dataParams.documentType).trim()) {
                dataParams.documentType = 'V';
            }
            if (!dataParams.clientType || !String(dataParams.clientType).trim()) {
                dataParams.clientType = 'Detal';
            }
            if (!dataParams.taxpayer || !String(dataParams.taxpayer).trim()) {
                dataParams.taxpayer = 'Ordinario';
            }
            if (!dataParams.address || !String(dataParams.address).trim()) {
                dataParams.address = 'No indicado';
            }

            const pendingPayment = new PendingPayments(dataParams);
            ...
            if ((!saleSaved) || (!wholesaleSaved) || (!pendingPaymentSaved)) {
                ...
                if (!client) {
                    const storeClient = new WholesaleClient(dataParams);

                    storeClient.pendingPayment = pendingPayment;

                    const storeClientSaved = await storeClient.save();
                    dataToDelete.WholesaleClient = storeClientSaved._id;
                }
                else {
            ...
            if (dataToDelete.PendingPayments) {
                await PendingPayments.findByIdAndDelete(dataToDelete.PendingPayments)
            }

            // Se elimina la cuenta pendiente colocada en el cliente y este sigue estando solvente
            if (dataToDelete.WholesaleClient) {
                await WholesaleClient.findOneAndUpdate({ _id: dataToDelete.WholesaleClient }, { pendingPayment: {}, isSolvent: true });
            }
 ...
 */
    update: async (id, dataParams) => {
        ...
        if (dataParams.extendCreditItems === true || dataParams.extendCreditItems === 'true') {
            return extendCreditItemsImpl(id, dataParams);
        }

        // Variables que necesitamos en todo el ámbito
        let pendingPayment;
        let pendingPaymentBackup;
        let orderId;
        ...
        try {
            ...
            orderId = parseInt(id);
            pendingPayment = await PendingPayments.findOne({ order: orderId });

            // Si no se encuentra por order, buscar por type 8
            if (!pendingPayment) {
                pendingPayment = await PendingPayments.findOne({
                    document: dataParams.document,
                    type: 8,
                    status: false // Solo pendientes no pagados
                }).sort({ createdDate: -1 }); // El más reciente

                if (!pendingPayment) {
                    throw 'No se encontró el pago pendiente';
                }
                orderId = pendingPayment.order; // Actualizar orderId con el encontrado
            }

            // 2. Backup del pago pendiente
            pendingPaymentBackup = await PendingPayments.findOne({ order: orderId }).lean();
            if (pendingPaymentBackup) {
            ...
            // 4. Actualizar valores del pago pendiente
            ...
            if (dataParams.changeDataPayment) {
                ...
                // Validación para abonos parciales
                if (!dataParams.isPayment) {
                    if (totalChangeInDollars > dataParams.total) {
                        throw 'ERROR: El cambio no puede ser mayor a lo abonado';
                    } else {
            ...
            // 8. Preparar estructura para rollback en caso de error
            let dataToDelete = {
                InventoryRecord: [],
                Sales: null,
                Wholesales: null,
            ...
            // 11. Actualizar pagos del pendiente
            ...
            if (!clientSaved) {
                throw 'Error actualizando el cliente';
            }

            // 13. Registrar movimientos de caja
            // BsS
            ...
            // 14. Registrar cambio en caja si existe
            if (dataParams.changeDataPayment) {
                let coin = dataParams.changeDataPayment.typeChangePayment;
                let value = dataParams.changeDataPayment.changeAmmountPayment;
                ...
                if (lastRecord) {
                    ...
                    if (coin === 2) boxData.valueDollar = dataParams.valueDollar;
                    if (coin === 3) boxData.valueEur = dataParams.valueEur;
                    if (coin === 4) boxData.valueCop = dataParams.valueCop;
            ...
            // 15. Eliminar ticket si es necesario
            if (dataParams.idTicket) {
                await Ticket.deleteOne({ _id: dataParams.idTicket });
            }
            ...
            // Rollback de todas las operaciones en caso de error
            try {
                if (dataToDelete.Sales) {
                    await Sales.findByIdAndDelete(dataToDelete.Sales);
                }
                ...
                if (dataToDelete.WholesaleClient) {
                    const client = await WholesaleClient.findOne({ document: dataParams.document });
                    if (client && pendingPaymentBackup) {
                ...
                if (pendingPayment && pendingPaymentBackup) {
                    const pendingToRestore = await PendingPayments.findOne({ order: pendingPayment.order });
                    if (pendingToRestore) {
                        Object.assign(pendingToRestore, pendingPaymentBackup);
                        await pendingToRestore.save();
                    }
                }
            } catch (rollbackError) {
    ...
    dataTable: async (dataParams) => {
        ...
        const pendingPayments = await PendingPayments.aggregate(stages);
