...
let ticketFiscalService = {
    ...
    create: async (ticketParam) => {
        ...
        const ticket = new TicketFiscal(ticketParam);
    ...
    update: async (id, ticketParam) => {
        ...
        const ticket = await TicketFiscal.findById(id);
    ...
    removeTicket: async (id, ticketParam) => {
        const ticketToDelete = await TicketFiscal.findOne({ _id: id });
        ...
        await TicketFiscal.deleteOne({ _id: id });

        //Obtener nuevamente todos los tickets
        let idAgency = ticketParam.id;
        const ticket = await TicketFiscal.find({agency:idAgency, isWholesale: ticketToDelete.isWholesale}).populate('agency','name').sort({createdDate: -1});
        return {
            results: ticket, 
        }
    },
    ...
    dataTable: async (ticketParam) => {
        ...
        // await TicketFiscal.deleteMany({ createdDate: { $lte: new Date(date) } });

        let idAgency = ticketParam.id;
        
        const ticket = await TicketFiscal.find({agency:idAgency, isWholesale: false}).populate('agency','name').sort({createdDate: -1});
        return {
            results: ticket, 
        }
    },
    ...
    dataTableWholesale: async (ticketParam) => {
        ...
        const ticket = await TicketFiscal.find({agency:idAgency, isWholesale: true}).populate('agency','name').sort({createdDate: -1});
...
module.exports = ticketFiscalService;