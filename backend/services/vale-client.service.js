...
let ValeClientService = {

    getAll: async () => {
        return await ValeClient.find();
    },
     ...
     */
    getById: async (id) => {
        return await ValeClient.findById(id);
    },
    ...
    create: async (clientParam) => {
        ...
        const client = await ValeClient.findOne({ document: clientParam.document })

        // Validar
        if (client) throw 'El usuario ya existe';

        // copiar propiedades de clientParam a newValeClient
        const newValeClient = new ValeClient(clientParam);

        //  Crear el nuevo cliente al mayor
        await newValeClient.save();
    },
    ...
    update: async (id, clientParam) => {
        const client = await ValeClient.findById(id);

        // Validar
        if (!client) throw 'Usuario no encontrado';
        if (client.document !== clientParam.document && await ValeClient.findOne({ document: clientParam.document })) {
    ...
    dataTable: async (clientParam) => {
        ...
        try {
            ...
            const clients = await ValeClient.aggregate(stages);
...
module.exports = ValeClientService;