...
const OrderHelper = db.OrderHelper;
...
let orderService = {
    ...
    create: async (orderParam) => {
        ...
        let lastOrder = await Order.findOne(query).sort({createdDate: -1})
        ...
        if (lastOrder){
            ...
            if (!lastOrder.wasConfirmed ){
                ...
                if (lastOrder.isModification){
                    ...
                    lastOrder = await Order.findOne(query).sort({createdDate: -1})
		...
		for(var key in orderProducts){
            ...
            let productToMatch = await productList.find( item => item.code === key);
    ...
    update: async (id, orderParam) => {
        ...
        if ((orderParam.user.role !== role.rol.Admin)  && (orderParam.user.role !== role.rol.Supervisor)){
            throw('El usuario no está autorizado para realizar confirmaciones')
        }
        
        // Se obtiene la orden que se va a confirmar
        let orderToConfirm = await Order.findById(id);
        ...
        if (orderParam.order){
            ...
            let originalOrder = await Order.findById(id);
            ...
            let newOrder = await orderService.create(newOrderParams);

            // La nueva orden creada, será la nueva orden a confirmar
            id = newOrder._id;

            orderToConfirm = await Order.findById(id);
        }
        ...
        if (orderToConfirm.isModification){
            ...
            let originalOrder = await Order.findOne(query).sort({createdDate: 1});

            // Se busca la ultima orden que corresponde con la modificacion
            let modificationOrder = await Order.findById(id).lean();
    ...
    orderTable: async (orderParam) => {
        ...
        const order = await Order.aggregate(stages);
    ...
    getOrderHelper: async () => {
        ...
        return await OrderHelper.find().lean();
    },
    ...
    setOrderHelper: async (orderParam) => {
        ...
        let existingRegister = await OrderHelper.findOne({ agency: orderParam.agency }).lean();

        if (existingRegister){
            await OrderHelper.findByIdAndDelete(existingRegister._id);
        }
        ...
        let newOrderHelper = new OrderHelper(orderHelper);
